...
class AssetGenerator:
    def __init__(self, project_folder="historic_facts", project_storage="store/projects/", script_path="scripts/prompt2.json"):
    ...
    def create_folder(self, script_path):
        ...
        # Get the actual project folder path instead of relying on script_path manipulation
    ...
    def generate_audio(self, script_json, folder_path, voice="en-US-JennyNeural"):
        ...
        for scene_number, scene in enumerate(script_json.get("scenes", []), start=1):
            print(f"Processing scene {scene_number}: {scene.get('title', 'Untitled')}")
            ...
            if not os.path.exists(audio_file):
                ...
                print(f"Audio file for scene {scene_number} already exists. Skipping generation.")

    def generate_images(self, script_json, folder_path, quantity_each=1):
        image_generator = ImageGenerator()

        for scene_number, scene in enumerate(script_json.get('scenes', []), start=1):
            print(f"Processing scene {scene_number}: {scene.get('story_title', 'Untitled')}")
            scene_folder = os.path.join(folder_path, str(scene_number))
            os.makedirs(scene_folder, exist_ok=True)
            ...
            for i, prompt in enumerate(image_prompts, start=1):
                for j in range(quantity_each):
                    ...
                    if not os.path.exists(image_file_path):
                        ...
                        print(f"Image {i}_{j+1} for scene {scene_number} already exists. Skipping generation.")
